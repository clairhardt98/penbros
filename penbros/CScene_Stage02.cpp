...
CScene_Stage02::CScene_Stage02()
	...
	, m_bGameOvered(false)
	, m_eCurState(STAGE2_STATE::PLAY)
{
	//이미지 로드하고, 애들 그려주기 전에 먼저 배경 그리면되겠네
	CResMgr::GetInst()->LoadTexture(L"Background1", L"Image\\Background1.bmp");
	m_pBGTex = CResMgr::GetInst()->FindTexture(L"Background1");

	m_pBGSound = CResMgr::GetInst()->LoadSound(L"Stage02BGM", L"Sound\\Stage2.wav");
}
...
void CScene_Stage02::Update()
{
	...
	switch (m_eCurState)
	{
	case STAGE2_STATE::ENTER:
		break;
	case STAGE2_STATE::PLAY:
...
void CScene_Stage02::GenerateNewMonsters()
{
	//몬스터 알 소환
...
void CScene_Stage02::MoveToNextStage()
{
	ChangeScene(SCENE_TYPE::STAGE_03);
}


void CScene_Stage02::UpdateState()
{

}

void CScene_Stage02::RevivePlayer()
{
	if (0 == CPlayer::GetHP())
	{
		SetGameOvered(true);
...
void CScene_Stage02::SetCleared(bool _b)
{
	m_bCleared = true;
	if (m_bCleared)
	{
		...
		CUI* pUI = (CUI*)GetUI();
		pUI->SetCleared(true);
		m_eCurState = STAGE2_STATE::CLEAR;
	}
}

void CScene_Stage02::SetGameOvered(bool _b)
{
	m_bGameOvered = _b;
	CUI* pUI = (CUI*)GetUI();
	pUI->SetGameOvered(true);
	m_eCurState = STAGE2_STATE::GAMEOVER;
}

void CScene_Stage02::InstantiatePlayer()
{
...
void CScene_Stage02::SpawnGhost()
{
...
void CScene_Stage02::ContinueGame()
{
